...
function Card({ item, dimmed, onHoverStart, onHoverEnd }: CardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-y-0 left-0 w-[55%] -translate-x-full -skew-x-12 bg-gradient-to-r from-transparent via-white/4.5 to-transparent transition-transform duration-700 ease-out group-hover:translate-x-[280%]"
      />
    ...
    >
      ...
      {(eyebrow || heading) && (
        <div className="relative mb-8 flex flex-col gap-1.5">
          {eyebrow ? (
            <p className="font-semibold text-[10px] text-cyan-500 uppercase tracking-[0.22em] dark:text-cyan-400/90">
              {eyebrow}
            </p>
          ) : null}
          {heading ? (
      ...
      {/* Card grid */}
      <div className={cn("relative grid", gridClassName)}>
        {items.map((item) => (
          <Card
